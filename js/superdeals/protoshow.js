	...
	var protoShow = Class
			.create({
				...
				initialize : function(element, options) {
					...
					}, options || {}); // We use Prototype's Object.extend() to
					// overwrite defaults with user
					// preferences

					// get/set various options
					this.element = $(element); // DOM element that contains the
					// slideshow
					this.slides = this.element.select(this.options.selector); // Elements
					// that
					// are
					// to
					// be
					// the
					// "Slides"
					this.slidesLength = this.slides.size(); // Total number of
					// Slides
					...
					this.mode = this[this.options.mode]; // Get play "mode"
					// (forward,
					// backward,
					// random...etc)
					this.autoPlay = this.options.autoPlay;
					this.progressTimer = this.options.progressTimer;
					this.showUniqueID = element; // get a unique ID based on
					// the id attr of the show
					// element
				...
				toggleMasterTimer : function(bln) {
					...
					if (bln) {
						// Check if custom interval has been defined by user as
						// data attribute in HTML
						var slideInterval = (this.slideIntervals[this.currentSlideID]) ? this.slideIntervals[this.currentSlideID]
								: this.interval;
				...
				forward : function(transTime) {
					...
					this
							.goMaster(this.currentSlideID + 1, transTime,
									"forward");
				},
				...
				goMaster : function(next, transTime, direction) {
					// Role: Master function - controls delegation of slide
					// swapping

					var _this = this;

					// First thing's first, we hault the show whatever the
					// circumstances
					this.toggleMasterTimer(false);
					...
					// Set the transistion speed to transTime arg (if set) else
					// fallback to standard transitionTime
					var transTime = (transTime) ? transTime
							: _this.transitionTime;

					this.toggleAnimating(true);
					this.setNextIndex(next); // set this.nextSlideID
					// correctly

					this.fireCustomEvent("protoShow:transitionStarted",
							transTime, direction, _this.nextSlideID);
					_this.updateNavigation(_this.currentSlideID,
							_this.nextSlideID);

					this
							.transitionType(
									this.currentSlideEle,
									this.nextSlideEle,
									{
										transitionTime : transTime,
										transitionFinish : function() { // pass
											// a
											// callback
											// to
											// ensure
											...
											_this.currentSlideEle
													.removeClassName('active-slide');
											_this.nextSlideEle
													.addClassName('active-slide');

											_this
													.updateCaptions(_this.nextSlideEle);
											_this
													.fireCustomEvent("protoShow:transitionFinished");
											_this.currentSlideID = _this.nextSlideID; // update
											// current
											// slide
											// to
											// be
											...
											if (_this.autoPlay && _this.running) {
												// if we're autoplaying and
												// we're not explicity stopped
												// otherwise show Master Timer
												// is not permitted to restart
												// itself
				...
				setupSlides : function() {
					...
					this.slides.each(function(e, index) {
						...
						slideInt = (slideInt && slideInt.blank()) ? undefined
								: slideInt; // check slideInt is not a blank
						// string

						_this.slideIntervals.push(slideInt); // push
						// intervals
						// into array
						// for use later
					});
					...
					this.slides[this.currentSlideID].show().addClassName(
							'active-slide');
				},
				...
				setupTransitions : function(transType) {
					...
					if (typeof (transType) == "function") { // user has defined
						// custom transition
						// function
						// If function then user has passed in custom transition
						// function to be used
						...
						if (transType === "slide") {
							...
							this.showWindow = this.element.down('.show').wrap(
									'div', {
										'class' : 'show-window'
									});
				...
				setupControls : function() {
					...
					this.protoControls = this.element.down('.proto-controls'); // Stop/Forward/Back
					// buttons

					if (typeof this.protoControls === "undefined") {

						var controlsEle = new Element('ol', {
							'class' : 'proto-controls'
						});
						var controlsTemplate = new Template(
								'<li class="#{htmlclass}"><a href="javascript:void(0)" title="#{title}">#{text}</a></li>');
						...
						controlsEle.insert(startStop, 'bottom').insert(
								backward, 'bottom').insert(forward, 'bottom');
						this.element.insert(controlsEle, 'bottom'); // add into
						// DOM
						this.protoControls = $(controlsEle); // extend the
						// DOM fragment
					}

					// If the controls already exists in the DOM
					this.controlStartStop = this.protoControls
							.down('.start-stop');
					...
					this.protoControls.on("click", ".proto-control", function(
							event, element) {
						event.stop();
						...
						if (element === _this.controlForward) {
							...
							if (_this.running) {
								_this.stop(); // if we're "Playing" then stop
								// the show
							} else {
								_this.play(); // else if we're not "Playing"
								// then start the show
							}
						}
				...
				setupNavigation : function() {
					...
					this.protoNavigation = this.element
							.down('.proto-navigation');

					if (typeof this.protoNavigation === "undefined") {
						var navEle = new Element('ol', {
							'class' : 'proto-navigation'
						});
						var navTemplate = new Template(
								'<li><a href="##{number}" title="Skip to Slide #{number}">#{number}</a></li>');

						this.slides.each(function(e, index) { // for each
							// slide in the
							// show create a
							// Nav <li>
							// using the
					...
					this.protoNavigation.down('li').addClassName(
							'current-slide');

					// define "lock" variable to stop abuse of controls
					var handlingClick = false;

					this.protoNavigation.on("click", "a", function(event,
							element) {
						event.stop();
						...
						var index = element.hash.substr(1, 2); // get the slide
						// ID from the
						// href hash
						// (eg: #3)
				...
				updateNavigation : function(current, next) {
					if (typeof this.protoNavigation !== "undefined") {
						this.protoNavigation.select('li')[current]
								.removeClassName('current-slide');
						this.protoNavigation.select('li')[next]
								.addClassName('current-slide');
					}
				},
				...
				setupCaptions : function() {
					...
					if (this.options.captions) {
						var captionEle = new Element('div', {
							'class' : 'slide-caption'
						});
						captionEle.hide();
				...
				updateCaptions : function(slide) {
					...
					if (nextCaption.replace(/^\s*|\s*$/g, '').length) { // check
						// that
						// the
						// attribute
						// has
					...
					} else { // if no caption is found then hide the caption
						// element
				...
				stopOnHover : function() {
					...
					if (this.options.pauseOnHover) {
						this.element.down('.show').observe('mouseenter',
								function() {
									_this.stop();
				...
				setupKeyboardControls : function() {
					...
					document.observe('keydown',
							function(key) {
								...
								// stop arrow keys from working when focused on
								// form items
								if ((!key.target.tagName
				...
				setupSwipeEvents : function() {
					...
					/*
					 * TOUCH START: Get and store the position of the initial
					 * touch
					 */
					...
					/*
					 * TOUCH MOVE: Called every time a user moves finger across
					 * the screen
					 */
				...
				toggleAnimating : function(bln) {
					// Role: toggles var to say whether animation is in progress
					// and manipulates DOM
					this.animating = bln;
					if (bln) {
				...
				updateControls : function(status) {
					if (this.options.controls) {
						...
						if (status) { // The show has been started so update
							// the button to "Pause"
							this.controlStartStop.down('a').update(
									_this.stopText);
						} else {
							// The show has been stopped so update the button to
							// "Play"
							this.controlStartStop.down('a').update(
									_this.playText);
						}
				...
				setupTimer : function() {
					// Role: creates the proto-progress-timer <canvas> element,
					// gets 2D Context and inserta into DOM

					this.progressTimerEle = document.createElement('canvas');
					if (this.progressTimerEle.getContext
							&& this.progressTimerEle.getContext('2d')) { // test
						// for
						// Canvas
						// support
						this.progressTimerEle.writeAttribute('class',
								'proto-progress-timer');
						this.progressTimerEle.width = 0;
						this.progressTimerEle.height = 0;
						this.element.insert(this.progressTimerEle, 'bottom');
						this.progressTimerCtx = this.progressTimerEle
								.getContext('2d');
					} else {
				...
				runProgressTimer : function() {
					// Role: runs & controls the animation of the "progress
					// timer"

					var _this = this;

					if (this.progressTimer) { // if user has set to use
						// progress timer and the
						// browser supports <canvas>

						this.progressTimerEle.show();

						// use Epoch time to ensure code executes in time
						// specified
						// borrowed from Emile JS
						// http://script.aculo.us/downloads/emile.pdf
						var start = (new Date).getTime();

						// we want the timer to finish slightly before the slide
						// transitions
						...
						this.progressTimerPE = new PeriodicalExecuter(
								function(pe) {
									_this.resetProgressTimer(); // clear the
									// canvas ready
									// for next
									// segment
									this.drawArc(_this.progressTimerCtx, 0,
											360, 'rgba(0,0,0,.2)'); // redraw
									// the black
									// bg circle

									var time = (new Date).getTime();
									var pos = time > finish ? 1
											: (time - start) / duration;

									// draw the arch passing in the ctx and the
									// degress of the arch
									this.drawArc(_this.progressTimerCtx, -5,
											Math.floor(((360) * pos)),
											'rgba(255,255,255,.8)', true);

									if ((!this.isPlaying()) || time > finish) { // if
										// we
										// are
										// stopped
										// or
										...
										// out
										pe.stop();
										_this.progressTimerEle
												.fade({
													duration : (_this.interval > 1000) ? (_this.interval / 8) / 1000
															: 0.2,
													afterFinish : function() {
														_this
																.resetProgressTimer();
				...
				drawArc : function(canvasCtx, startAngle, endAngle, strokeStyle) {
					// Role: utility function for drawing archs on <canvas>
					// elements
					...
					ctx.arc(15, 15, 10, (Math.PI / 180) * (startAngle - 90),
							(Math.PI / 180) * (endAngle - 90), false);
					ctx.stroke();
				...
				/*
				 * reportSlides: function() { console.log("Current slide: " +
				 * this.currentSlideID); console.log("Next slide: " +
				 * this.nextSlideID); },
				 */
